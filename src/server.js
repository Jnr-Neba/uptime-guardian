...
collectDefaultMetrics({ prefix: 'uptime_guardian_' })

const checksTotal = new client.Counter({
  name: 'uptime_guardian_checks_total',
  help: 'Total number of endpoint health checks performed',
  labelNames: ['status']
})

const checkDuration = new client.Histogram({
  name: 'uptime_guardian_check_duration_ms',
  help: 'Duration of endpoint health checks in milliseconds',
  labelNames: ['status'],
...
app.post('/api/check', (req, res) => {
  ...
  const request = clientHttp.get(url, { timeout: 5000 }, (response) => {
    const latency = Date.now() - start
    const healthy = response.statusCode >= 200 && response.statusCode < 400
    response.resume()

    checksTotal.inc({ status: healthy ? 'success' : 'failure' })
    checkDuration.observe({ status: healthy ? 'success' : 'failure' }, latency)
  ...
  request.on('timeout', () => {
    request.destroy()
    checksTotal.inc({ status: 'failure' })
    checkDuration.observe({ status: 'failure' }, 5000)
  ...
  request.on('error', (err) => {
    const latency = Date.now() - start
    checksTotal.inc({ status: 'failure' })
    checkDuration.observe({ status: 'failure' }, latency)
    res.json({ url, status: null, latency_ms: latency, healthy: false, error: err.message, checked_at: new Date().toISOString() })
  })
})
...
app.listen(3000, () => {
  ...
  console.log('[uptime-guardian] Metrics:   http://localhost:3000/metrics')
})